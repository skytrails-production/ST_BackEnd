...
exports.searchTboandGrnCityList = async  (req, res) => {
  ...
  try {
    ...
    cityList=cityList.sort((a, b) => {
      // Replace 'CountryCode' with the actual property in your cityData model representing country code
      const acountrycode = a?.tboCountryCode?.trim().toUpperCase()||a?.grnCountryCode?.trim().toUpperCase();
      const bcountrycode = b?.tboCountryCode?.trim().toUpperCase()||b?.grnCountryCode?.trim().toUpperCase();
      const userCountryCode = userLocation.country.trim().toUpperCase();
...
const uniqueHotels = Array.from(minPriceMap, ([hotelName, entry]) => {
    ...
    return {
        ...
        // price: entry.price,
        ...Object.fromEntries(Object.entries(entry).filter(([key]) => key !== 'HotelName' && key !== 'name' && key !== 'Price.PublishedPrice' && key !== 'min_rate.price'))
    };
});
