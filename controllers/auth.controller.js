...
const { aggregatePaginateHotelBookingList, aggregatePaginateHotelBookingList1, findhotelBooking, findhotelBookingData, deletehotelBooking, updatehotelBooking, hotelBookingList, countAgentTotalBooking } = hotelBookingServicess;
const { createUser, findUser, getUser, findUserData, updateUser, paginateUserSearch, countTotalUser,aggregatePaginateUser } = userServices;
...
const { aggregatePaginateGetBooking,aggregatePaginateGetBooking1,countTotalUserFlightBooking } = userflightBookingServices;
...
const { createUserBusBooking, findUserBusBooking, getUserBusBooking, findUserBusBookingData, deleteUserBusBooking, userBusBookingList, updateUserBusBooking, paginateUserBusBookingSearch,countTotalUserBusBooking } = userBusBookingServices
...
const {  aggregatecancelFlightBookingsList,getAgentHotelCancelRequesrByAggregate,getBusCancellationAgent,countTotalAgentBusCancelled,countTotalAgentHotelCancelled,countTotalAgentcancelFlightBookings } = cancelBookingServices;
const {changeUserBookingServices}=require("../services/btocServices/changeRequestServices");
const {flightchangeRequestUserList,hotelchangeRequestUserList,buschangeRequestUserList,countChangeBusRequest,countChangeHotelRequest,countChangeFlightRequest}=changeUserBookingServices;
...
exports.adminDashBoard = async (req, res, next) => {
  try {
    var result = {}
    const userFlightBooking=await countTotalUserFlightBooking({status:status.ACTIVE});
    const userBusBooking=await countTotalUserBusBooking({status:status.ACTIVE});
    const userHotelBooking=await countTotalhotelBooking({status:status.ACTIVE});
    const agentHotelBooking=await countAgentTotalBooking({status:status.ACTIVE});
    const agentFlightBooking=await flightModel.countDocuments({status:status.ACTIVE});
    const agentBusBooking=await busBookingModel.countDocuments({status:status.ACTIVE });
    const agentCancelBusBookingCount=await countTotalAgentBusCancelled({status:status.ACTIVE});
    const agentCancelHotelBookingCount=await countTotalAgentHotelCancelled({status:status.ACTIVE });
    const agentCancelFlightBookingCount=await countTotalAgentcancelFlightBookings({status:status.ACTIVE });
    const userCancelBusBookingCount=await countTotalBusCancelled({status:status.ACTIVE });
    const userCancelHotelBookingCount=await countTotalHotelCancelled({status:status.ACTIVE});
    const userCancelFlightBookingCount=await countTotalcancelFlightBookings({status:status.ACTIVE });
    const userChangeBusBookingCount=await countChangeBusRequest({status:status.ACTIVE});
    const userChangeHotelBookingCount=await countChangeHotelRequest({status:status.ACTIVE});
    const userChangeFlightBookingCount=await countChangeFlightRequest({status:status.ACTIVE});
    const agentChangeBusRequest=await countTotalchangeBusRequest({status:status.ACTIVE});
    const agentChangeHotelRequest=await countTotalchangeHotelRequest({status:status.ACTIVE});
    const agentChangeFlightRequest=await countTotalchangeRequest({status:status.ACTIVE});
    ...
    result.totalCancelled=agentCancelBusBookingCount+agentCancelHotelBookingCount+agentCancelFlightBookingCount+userCancelBusBookingCount+userCancelHotelBookingCount+userCancelFlightBookingCount;
    result.totalChangeRequests=agentChangeBusRequest+agentChangeHotelRequest+agentChangeFlightRequest+userChangeBusBookingCount+userChangeHotelBookingCount+userChangeFlightBookingCount
    return res.status(statusCode.OK).send({ message: responseMessage.DATA_FOUND, result: result });
  } catch (error) {
    console.log("error=======>>>>>>", error);
    return next(error);
...
exports.getAllUsers=async(req,res,next)=>{
  try {
    ...
    res.status(statusCode.OK).send({statusCode:statusCode.OK,responseMessage:responseMessage.DATA_FOUND,result:result})
  } catch (error) {
...
try {
  const { userId, actionStatus } = req.body;
  const iSubAdminExist = await findSubAdmin({ _id: userId, status: status.ACTIVE });
    if (!iSubAdminExist) {
    ...
    const updateData=await updateSubAdmin({_id:iSubAdminExist._id},{status:actionStatus});
    if (actionStatus === status.ACTIVE) {
