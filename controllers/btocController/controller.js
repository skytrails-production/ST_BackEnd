  ...
  try {
    ...
    if(mobileNumber==="9999123232"){
      let updatedNumber = await updateUser(
        { "phone.mobile_number": mobileNumber, status: status.ACTIVE},{"phone.mobile_number": mobileNumber,otpVerified:false,otpExpireTime:null,approveStatus:"APPROVED"});
...
exports.verifyUserOtp = async (req, res, next) => {
  try {
    ...
    if (isUserExist.otp !== otp) {
      ...
      const updateStaticUser = await updateUser({ _id: isUserExist._id, status: status.ACTIVE },{ otpVerified: true,firstTime:false});
...
exports.verifyUserOtpWithMail = async (req, res, next) => {
  try {
